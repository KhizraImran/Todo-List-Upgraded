...
let deleteTask = async () => {
  if (todoList.length === 0) {
    console.log(
      "Your Todo list is empty. Please write something before deleting a task \n"
    );
    return;
  }
  await viewTask();
...
let updateTask = async () => {
  if (todoList.length === 0) {
    console.log(
      "Your Todo list is empty. Please write something  before updating a task! (Thank you)\n"
    );
    return;
  }
  await viewTask();
